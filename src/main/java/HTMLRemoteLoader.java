...
public class HTMLRemoteLoader {
	...
	public static void aggregate() throws SQLException {
		...
		stm.executeUpdate(
				"INSERT INTO \"PUBLIC\".\"COVID\" (COUNTRY,TOTAL_CASES,NEW_CASES,TOTAL_DEATHS,NEW_DEATHS,TOTAL_RECOVERED,ACTIVE_CASES,SERIOUS_CRITICAL,TOT_CASES_1M_POP,DEATHS_1M_POP,TOTAL_TESTS,TESTS_1M_POP,CONTINENT) \r\n"
						...
						+ "FROM CSVREAD('tmp/covid_world_today.csv','COUNTRY;TOTAL_CASES;NEW_CASES;TOTAL_DEATHS;NEW_DEATHS;TOTAL_RECOVERED;ACTIVE_CASES;SERIOUS_CRITICAL;TOT_CASES_1M_POP;DEATHS_1M_POP;TOTAL_TESTS;TESTS_1M_POP;CONTINENT','fieldSeparator=;');");
		// AGGREGATE DATA
		ResultSet rs = stm.executeQuery("SELECT *, CURRENT_DATE() AS DATE FROM COVID \r\n"
				+ "WHERE COUNTRY IN ('Italy','Germany','Spain','France', 'UK','USA','Switzerland','Austria','Japan','S. Korea') \r\n"
	...
	public static void htmlToCsv() {
		try {
			 ...
			 * + ";"; } else { thstr += th.text() + "\n"; } counter++; } thstr =
			 * thstr.trim().replaceAll(",", "_").replaceAll("/", "_").replaceAll("
			 * ", "_")+"\n";
			 */
	...
	public static Document getHtmlDoc() throws MalformedURLException, IOException, ProtocolException {
		...
		//Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("proxy-here", 8080));
